...
class _SimpleGraph:
    ...
    @property
    def vs(self) -> list[_SimpleVertex]:
        return [_SimpleVertex(index) for index in self._vertices]

    def add_vertex(self, vertex_id: int) -> None:
        if vertex_id not in self._vertices:
            self._vertices.append(vertex_id)

    def delete_vertices(self, vertex_id: int) -> None:
    ...
    def delete_edges(self, edges: list[tuple[int, int]]) -> None:
        for edge in edges:
            self._edges.discard(tuple(edge))
...
class Neo4jHandler:
    ...
    def __init__(self, nei4j_config: Neo4jConfig):
        if GraphDatabase is None:
            raise ModuleNotFoundError("neo4j is required for the neo4j graph backend.")
...
class AgentGraph:
    ...
    def __init__(
        ...
        if self.backend == "igraph":
            self.graph = ig.Graph(directed=True) if ig is not None else _SimpleGraph()
        else:
            assert neo4j_config is not None
            assert neo4j_config.is_valid()
            self.graph = Neo4jHandler(neo4j_config)
        self.agent_mappings: dict[int, Any] = {}
    ...
    def get_agents(
            self,
            agent_ids: list[int] = None) -> list[tuple[int, Any]]:
        if agent_ids:
