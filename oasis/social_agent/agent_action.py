...
from oasis.localization import is_zh_locale, localize_tool_schema


class SocialAction:
    _tool_schema_cache: dict[tuple[str, int], list[dict]] = {}
    _filtered_tool_schema_cache: dict[
        tuple[str, int, tuple[str, ...] | None],
        list[dict],
    ] = {}
    _content_length_action_names: tuple[str, ...] = (
        "create_post",
        "quote_post",
        "create_comment",
    )

    _action_function_names: tuple[str, ...] = (
        "create_post",
        "like_post",
        "repost",
        "quote_post",
    ...
    def _iter_action_functions(self):
        for name in self._action_function_names:
            yield getattr(self, name)

    def _apply_content_length_to_tool_schema(self, tool: dict) -> dict:
        function = tool.get("function", {})
        if function.get("name") not in self._content_length_action_names:
            return tool

        length = str(self.max_content_length)
        description = function.get("description")
        if isinstance(description, str):
            function["description"] = description.replace("280", length)
            if "280" not in description:
                if is_zh_locale(self.locale):
                    ...
                    function["description"] = (
                        f"{function['description']} "
                        f"Maximum weighted content length: {length}. "
                        "Single-byte characters count as 1; CJK characters "
                        "count as 2."
                    )
        ...
        if isinstance(content_schema, dict):
            content_description = content_schema.get("description")
            if isinstance(content_description, str):
                content_schema["description"] = content_description.replace(
                    "280",
                    length,
                )
                if "280" not in content_description:
                    if is_zh_locale(self.locale):
                        content_schema["description"] = (
                            f"{content_schema['description']} "
                            f"最大加权内容长度：{length}。"
                            "单字节字符计 1，CJK 字符计 2。"
                        )
                    else:
                        content_schema["description"] = (
                            f"{content_schema['description']} "
                            f"Maximum weighted content length: {length}. "
                            "Single-byte characters count as 1; CJK "
                            "characters count as 2."
                        )

        return tool

    def get_openai_function_list(self) -> list[dict]:
        locale_key = str(self.locale or "en").lower().replace("-", "_")
        cache_key = (locale_key, self.max_content_length)
        if cache_key not in self._tool_schema_cache:
            tools = [
                FunctionTool(func).get_openai_tool_schema()
                for func in self._iter_action_functions()
            ]
            self._tool_schema_cache[cache_key] = [
                self._apply_content_length_to_tool_schema(
                    localize_tool_schema(tool, self.locale)
                )
                for tool in tools
            ]
        return self._tool_schema_cache[cache_key]

    def get_openai_function_list_for_actions(
        self,
        ...
        cache_key = (locale_key, self.max_content_length, action_names)
        if cache_key not in self._filtered_tool_schema_cache:
            all_tools = self.get_openai_function_list()
            if action_names is None:
                self._filtered_tool_schema_cache[cache_key] = all_tools
            else:
                action_name_set = set(action_names)
                self._filtered_tool_schema_cache[cache_key] = [
                    tool
                    for tool in all_tools
                    if tool["function"]["name"] in action_name_set
                ]
        return self._filtered_tool_schema_cache[cache_key]

    def get_function_tool_list(self) -> list[FunctionTool]:
        schemas_by_name = {
            tool["function"]["name"]: tool
            for tool in self.get_openai_function_list()
        }
        return [
            FunctionTool(
                func,
                openai_tool_schema=schemas_by_name[func.__name__],
            )
            for func in self._iter_action_functions()
        ]

    def get_function_tool_list_for_actions(
        self,
        available_actions: list[ActionType] | None = None,
    ) -> list[FunctionTool]:
        function_tools = self.get_function_tool_list()
        ...
        return [
            ...
            if tool.get_function_name() in action_names
        ]
    ...
    async def do_nothing(self, post_id: int):
        """Do nothing in response to a specific visible post.

        Use this tool when the agent decides not to interact after seeing a
        particular post. The `post_id` argument is required for behavior
        consistency checking and must be selected from the currently visible
        posts in [Posts]. The platform execution ignores this argument and only
        records that the agent did nothing.

        Args:
            post_id (int): Required. The ID of the visible post that triggered
                the decision to do nothing. Choose it from [Posts].
    ...
    async def create_post(self, content: str, mentioned_user_ids: Optional[list[int]] = None):
        r"""Create a new post with the given content.

        This method invokes an asynchronous action to create a new post based
        on the provided content. The content must stay within the configured
        platform content length limit.

        Including user IDs in the 'mentioned_user_ids' field ensures that the
        specified users are notified of this post.

        Upon successful execution, it returns a dictionary indicating success
        and the ID of the newly created post.

        Args:
            content (str): The content of the post as a string.
                The content is limited to prevent exceeding platform limits.
                Exceeding this limit will result in an error.
            mentioned_user_ids (Optional[list[int]]): List of user IDs to be mentioned in the post.
                Each ID should be an integer representing a valid user.
                Defaults to None, which means no users are mentioned.
        ...
        """
        post_message = (content, mentioned_user_ids or [])
        return await self.perform_action(post_message, ActionType.CREATE_POST.value)
    ...
    async def quote_post(self, post_id: int, content: str, mentioned_user_ids: Optional[list[int]] = None):
        r"""Quote a specified post with a given quote content.

        This method invokes an asynchronous action to quote a specified post
        with commentary or reaction. The quote content must stay within the
        configured platform content length limit.

        Including user IDs in the 'mentioned_user_ids' field ensures that the
        specified users are notified of this quote.

        Upon successful execution, it returns a dictionary indicating success
        and the ID of the newly created quote post.

        Args:
            post_id (int): The ID of the post to be quoted as an integer.
                This identifies which post the user is quoting.
            content (str): The quote commentary content as a string.
                This is the user's comment or reaction to the original post.
                Exceeding this limit will result in an error.
            mentioned_user_ids (Optional[list[int]]): List of user IDs to be mentioned in the quote.
                Each ID should be an integer representing a valid user.
                Defaults to None, which means no users are mentioned.
        ...
        Note:
            ...
            result in a failure.
        """
        quote_message = (post_id, content, mentioned_user_ids or [])
    ...
    async def follow(self, followee_id: int, post_id: int):
        r"""Follow a user in response to a specific visible post.
        ...
        The `post_id` argument is required for behavior consistency checking
        and must be selected from the currently visible posts in [Posts]. The
        platform execution ignores `post_id` and only uses `followee_id`.

        Args:
            followee_id (int): The user ID of the user to be followed.
                Prefer the author/user_id of the triggering visible post.
            post_id (int): Required. The ID of the visible post that triggered
                the follow action. Choose it from [Posts].
    ...
    async def create_comment(self, post_id: int, content: str, mentioned_user_ids: Optional[list[int]] = None):
        r"""Create a new comment for a specified post.

        This method creates a new comment based on the provided content and
        associates it with the given post ID. Comments must stay within the
        configured platform content length limit.

        Including user IDs in the 'mentioned_user_ids' field ensures that the
        specified users are notified of this comment. Upon successful execution,
        ...
        Args:
            post_id (int): The ID of the post to which the comment is to be
                added. This should be an integer representing a valid post ID.
            content (str): The content of the comment to be created. This is
                limited by the configured platform content length.
            mentioned_user_ids (Optional[list[int]]): A list of user IDs to be
                mentioned in the comment. Defaults to None.
        ...
        """
        comment_message = (post_id, content, mentioned_user_ids or [])
    ...
    async def view_comments(self,post_id:int):
        return await self.perform_action(post_id,"view_comment")
