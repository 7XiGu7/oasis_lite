...
from oasis.social_platform.typing import ActionType, RecsysConfig, RecsysType
from oasis.localization import is_zh_locale, localize_result_text, platform_display_name
...
if "sphinx" not in sys.modules:
    ...
    platform_log_path = os.getenv(
        "OASIS_PLATFORM_LOG_PATH",
        f"{log_dir}/social.twitter-{now}.log",
    )
    Path(platform_log_path).parent.mkdir(parents=True, exist_ok=True)
    file_handler = logging.FileHandler(platform_log_path)
...
class Platform:
    ...
    # Default content length limit (280 weighted characters)
    # Single-byte characters count as 1, multi-byte characters (CJK) count as 2
    ...
    def __init__(
        ...
        max_search_post_count: int = 3,
        max_search_user_count: int = 3,
        use_openai_embedding: bool = False,
        time_top_n_comments:  int = None,
        locale: str = "en",
        ...
        if recsys_config is None:
            recsys_config = RecsysConfig(
                ...
                time_top_n_comments=time_top_n_comments,
                use_openai_embedding=use_openai_embedding,
            )
        self.recsys_config = recsys_config.validated()
        self.recsys_type = self.recsys_config.recsys_kind
        self.locale = locale
        ...
        self.platform_name = platform_display_name(platform_name, locale)
        recsys_kind = self.recsys_type
        ...
        self.recsys_available_device = self.recsys_config.available_device

        self.channel = channel or Channel()

        self.recsys_type = recsys_kind
        ...
        self.refresh_rec_post_count = self.recsys_config.refresh_rec_post_count
        # The number of posts returned at once from posts made by followed
        # users, ranked by like counts
        self.following_post_count = self.recsys_config.following_post_count
        # The maximum number of posts per user in the recommendation
        # table (buffer)
        self.max_rec_post_len = self.recsys_config.max_rec_post_len
        if self.refresh_rec_post_count > self.max_rec_post_len:
            twitter_log.warning(
                "refresh_rec_post_count (%s) is greater than "
                "max_rec_post_len (%s); refresh will return at most the "
                "available cached recommendations.",
                self.refresh_rec_post_count,
                self.max_rec_post_len,
            )
        # The maximum number of posts returned by the search function
        self.max_search_post_count = max_search_post_count
        # The maximum number of users returned by the search function
        self.max_search_user_count = max_search_user_count
        self.use_openai_embedding = self.recsys_config.use_openai_embedding

        # Parameters for the platform's internal trending rules
        self.trend_num_days = 7
        self.trend_top_k = 1

        # Number of comments displayed for each post in chronological order
        self.time_top_n_comments = self.recsys_config.time_top_n_comments
    ...
    async def running(self):
        while True:
            ...
            if action_function:
                ...
                result = localize_result_text(result, self.locale)
    ...
    async def _build_post_attr(self, post_content: str) -> dict[str, Any]:
        return {}
    ...
    def _validate_content_length(self, content: str) -> dict:
        r"""Validate content length according to Twitter-like character counting rules.

        Character counting rules:
        - Single-byte characters (English, numbers, basic punctuation, etc.): 1 character
        - Multi-byte characters (CJK: Chinese, Japanese, Korean): 2 characters

        Args:
            content (str): The content to validate

        Returns:
            dict: {'success': bool, 'char_count': int, 'message': str (if error)}
        """
        if not content:
        ...
        char_count = 0
        for char in content:
            # Check if character is a multi-byte character (CJK)
            # CJK Unified Ideographs ranges: \u4E00-\u9FFF
            # CJK Unified Ideographs Extension A: \u3400-\u4DBF
            # CJK Unified Ideographs Extension B: \u20000-\u2A6DF
            # Japanese Hiragana and Katakana: \u3040-\u309F, \u30A0-\u30FF
            # Korean Hangul: \uAC00-\uD7AF
            if ord(char) >= 0x4E00 and ord(char) <= 0x9FFF:  # CJK Unified Ideographs
                char_count += 2
            elif ord(char) >= 0x3400 and ord(char) <= 0x4DBF:  # CJK Unified Ideographs Extension A
                char_count += 2
            elif ord(char) >= 0x3040 and ord(char) <= 0x309F:  # Hiragana
                char_count += 2
            elif ord(char) >= 0x30A0 and ord(char) <= 0x30FF:  # Katakana
                char_count += 2
            elif ord(char) >= 0xAC00 and ord(char) <= 0xD7AF:  # Hangul
                char_count += 2
            else:
                char_count += 1
        ...
        if char_count > self.max_content_length:
            return {
                ...
                "message": f"Content exceeds character limit. Length: {char_count}, Limit: {self.max_content_length}"
            }
    ...
    async def refresh(self, agent_id: int):
        ...
        try:
            ...
            if self.recsys_type != RecsysType.REDDIT:
                ...
                if len(following_posts_ids) > self.following_post_count:
                    following_posts_ids = random.sample(
                        following_posts_ids, self.following_post_count)
            ...
            # recent_interactions = self.pl_utils._get_interaction(user_id,self.time_top_n_comments)
            ...
            twitter_log.info(f"fail to refresh posts,error: {e}")
            return {"success": False, "error": str(e)}
    ...
    async def update_rec_table(self):
        ...
        if self.recsys_type == RecsysType.RANDOM:
            new_rec_matrix = rec_sys_random(post_table, rec_matrix,
                                            self.max_rec_post_len)
        elif self.recsys_type in (RecsysType.TWITTER, RecsysType.WEIBO):
            ...
            try:
                ...
                new_rec_matrix = rec_sys_personalized_twh(
                    ...
                    self.recsys_available_device,
                    model_name_or_path=self.recsys_config.model_name_or_path,
                    embedding_batch_size=(
                        self.recsys_config.resolved_embedding_batch_size()
                    ),
                    openai_embedding_batch_size=(
                        self.recsys_config.resolved_openai_embedding_batch_size()
                    ),
                    coarse_filter_size=self.recsys_config.coarse_filter_size,
                    use_openai_embedding=self.use_openai_embedding,
                )
            except Exception as e:
                twitter_log.error(e)
                twitter_log.info("No new posts found, skip updating the rec table.")
    ...
    async def create_post(self, agent_id: int, post_message: tuple):
        content, mentioned_user_ids = post_message
        if self.recsys_type == RecsysType.REDDIT:
            current_time = self.sandbox_clock.time_transfer(
                datetime.now(), self.start_time)
        else:
        ...
        try:
            ...
            validation_result = self._validate_content_length(content)
            if not validation_result["success"]:
                return {"success": False, "error": validation_result["message"]}

            post_attr = await self._build_post_attr(content)
            post_attr_str = json.dumps(post_attr, ensure_ascii=False)

            post_insert_query = (
                "INSERT INTO post (user_id, content, created_at, num_likes, "
                "num_dislikes, num_shares, post_attr) VALUES (?, ?, ?, ?, ?, ?, ?)")
            self.pl_utils._execute_db_command(
                post_insert_query, (user_id, content, current_time, 0, 0, 0, post_attr_str),
                commit=True)
            post_id = self.db_cursor.lastrowid

            # 如果存在mentioned_user_ids，则添加记录
            if mentioned_user_ids:
                for mentioned_user_id in mentioned_user_ids:
                    mention_insert_query = (
                        "INSERT INTO mention (user_id, post_id, mentioned_user_id, created_at) VALUES (?, ?, ?, ?)")
                    self.pl_utils._execute_db_command(
                        mention_insert_query,
                        (user_id, post_id, mentioned_user_id, current_time),
                        commit=True,
                    )
                    mention_id = self.db_cursor.lastrowid
                    action_info = {"mention_id":mention_id,"post_id": post_id,"mentioned_user_id": mentioned_user_id,}
                    self.pl_utils._record_trace(user_id, ActionType.MENTION.value,
                                                action_info, current_time)
                    self.pl_utils._record_interaction(user_id, post_id, None, mentioned_user_id, 1,ActionType.MENTION.value)
    ...
    async def repost(self, agent_id: int, post_id: int):
        ...
        try:
            ...
            if not post_type_result:
                ...
                get_post_attr_query = "SELECT post_attr FROM post WHERE post_id = ?"
                self.pl_utils._execute_db_command(get_post_attr_query, (post_id,))
                source_post_attr = self.db_cursor.fetchone()
                post_attr_str = source_post_attr[0] if source_post_attr and source_post_attr[0] else json.dumps({})

                self.pl_utils._execute_db_command(
                    post_insert_query, (user_id, post_id, current_time,post_attr_str),
                    commit=True)
                ...
                get_post_attr_query = "SELECT post_attr FROM post WHERE post_id = ?"
                self.pl_utils._execute_db_command(get_post_attr_query,
                ...
                self.pl_utils._execute_db_command(
                    post_insert_query,
                    (user_id, post_type_result['root_post_id'], current_time, post_attr_str),
                    commit=True)
                self.pl_utils._execute_db_command(
                    update_shares_query, (post_type_result['root_post_id'], ),
                    commit=True)
                root_post_id = post_type_result['root_post_id']
            ...
            self.pl_utils._record_trace(user_id, ActionType.REPOST.value,
                                        action_info, current_time)
            self.pl_utils._record_interaction(user_id, new_post_id, None, root_post_id, 0,
    ...
    async def quote_post(self, agent_id: int, quote_message: tuple):
        post_id, quote_content, mentioned_user_ids = quote_message
        if self.recsys_type == RecsysType.REDDIT:
            current_time = self.sandbox_clock.time_transfer(
                datetime.now(), self.start_time)
        else:
        ...
        try:
            ...
            validation_result = self._validate_content_length(quote_content)
            if not validation_result["success"]:
                return {"success": False, "error": validation_result["message"]}

            post_attr = await self._build_post_attr(quote_content)
            post_attr_str = json.dumps(post_attr, ensure_ascii=False)
            ...
            post_insert_query = (
                ...
                "content, quote_content, created_at, post_attr) VALUES (?, ?, ?, ?, ?, ?)")
            update_shares_query = (
                "UPDATE post SET num_shares = num_shares + 1 WHERE post_id = ?"
            )
            root_post_id = None
            if not post_type_result:
                ...
                self.pl_utils._execute_db_command(
                    post_insert_query, (user_id, post_id, post_content,
                                        quote_content, current_time, post_attr_str),
                    commit=True)
            ...
            if mentioned_user_ids:
                for mentioned_user_id in mentioned_user_ids:
                    ...
                    self.pl_utils._execute_db_command(
                        mention_insert_query,
                        (user_id, new_post_id, mentioned_user_id, current_time),
                        commit=True,
                    )
                    mention_id = self.db_cursor.lastrowid
                    action_info = {"mention_id": mention_id, "post_id": new_post_id,
                                   "mentioned_user_id": mentioned_user_id, }
                    self.pl_utils._record_trace(user_id, ActionType.MENTION.value,
                                                action_info, current_time)

                    self.pl_utils._record_interaction(user_id, new_post_id, None, mentioned_user_id, 1,ActionType.MENTION.value)
    ...
    async def like_post(self, agent_id: int, post_id: int):
        ...
        try:
            ...
            self.pl_utils._record_trace(user_id, ActionType.LIKE_POST.value,
                                        action_info, current_time)
            # 记录交互记录
            self.pl_utils._record_interaction(user_id, post_id, None, post_id, 0,
    ...
    async def search_posts(self, agent_id: int, query: str):
        ...
        try:
            ...
            sql_query = (
                ...
                "num_shares, post_attr FROM post WHERE content LIKE ? OR CAST(post_id AS "
            ...
            results = random.sample(
                rows,
                min(self.max_search_post_count, len(rows)),
            )
    ...
    async def search_user(self, agent_id: int, query: str):
        ...
        try:
            ...
            results = random.sample(
                rows,
                min(self.max_search_user_count, len(rows)),
            )
    ...
    async def follow(self, agent_id: int, followee_id: int):
        ...
        try:
            ...
            self.pl_utils._record_trace(user_id, ActionType.FOLLOW.value,
                                        action_info, current_time)
            self.pl_utils._record_interaction(user_id, None, None, followee_id, 0,
    ...
    async def trend(self, agent_id: int):
        ...
        try:
            ...
            sql_query = """
                SELECT post_id, user_id, original_post_id, content,
                quote_content, created_at, num_likes, num_dislikes,
                num_shares, post_attr FROM post
                WHERE created_at >= ?
    ...
    async def create_comment(self, agent_id: int, comment_message: tuple):
        post_id, content, mentioned_user_ids = comment_message
        if self.recsys_type == RecsysType.REDDIT:
            current_time = self.sandbox_clock.time_transfer(
                datetime.now(), self.start_time)
        else:
        ...
        try:
            ...
            if not post_type_result:
                return {"success": False, "error": f"Post not found. post_id: {post_id}"}
            ...
            if mentioned_user_ids:
                for mentioned_user_id in mentioned_user_ids:
                    mention_insert_query = (
                        "INSERT INTO mention (user_id, post_id, comment_id, mentioned_user_id, created_at) VALUES (?, ?, ?, ?, ?)")
                    self.pl_utils._execute_db_command(
                        mention_insert_query,
                        (user_id, post_id, comment_id, mentioned_user_id, current_time),
                        commit=True,
                    )
                    mention_id = self.db_cursor.lastrowid
                    action_info = {"mention_id": mention_id, "post_id": post_id, "comment_id": comment_id,
                                   "mentioned_user_id": mentioned_user_id, }
                    self.pl_utils._record_trace(user_id, ActionType.MENTION.value,
                                                action_info, current_time)
                    self.pl_utils._record_interaction(user_id, post_id, comment_id, mentioned_user_id, 1,
                                                      ActionType.MENTION.value)
            ...
            self.pl_utils._record_interaction(user_id, post_id, comment_id, post_id, 0,
                                              ActionType.CREATE_COMMENT.value)
    ...
    async def view_comments(self, agent_id: int, post_id: int):
        if self.recsys_type == RecsysType.REDDIT:
            current_time = self.sandbox_clock.time_transfer(
                datetime.now(), self.start_time)
        else:
        ...
        try:
            ...
            query_args = (post_id,)
            if self.time_top_n_comments is not None:
                sql_query += " LIMIT ?"
                query_args = (post_id, self.time_top_n_comments)
            # Rewrite to use the execute_db_command method
            self.pl_utils._execute_db_command(
                sql_query,
                query_args,
            ...
            } for comment_id, post_id, user_id, content, created_at, num_likes,
                     num_dislikes in results]

            action_info = {"post_id": post_id, "comment_count": len(comments)}
            self.pl_utils._record_trace(user_id, ActionType.VIEW_COMMENT.value,
                                        action_info, current_time)
