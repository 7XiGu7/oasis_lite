...
def get_twhin_tokenizer(model_name_or_path: str = "Twitter/twhin-bert-base"):
    global twhin_tokenizer
    if model_name_or_path not in _twhin_tokenizers:
        from transformers import AutoTokenizer
        _twhin_tokenizers[model_name_or_path] = AutoTokenizer.from_pretrained(
            pretrained_model_name_or_path=model_name_or_path,
            model_max_length=512)
    twhin_tokenizer = _twhin_tokenizers[model_name_or_path]
    return twhin_tokenizer


def get_twhin_model(device, model_name_or_path: str = "Twitter/twhin-bert-base"):
    global twhin_model
    cache_key = (model_name_or_path, str(device))
    if cache_key not in _twhin_models:
        from transformers import AutoModel
        loaded_model = AutoModel.from_pretrained(
            pretrained_model_name_or_path=model_name_or_path
        ).to(device)
        loaded_model.eval()
        _twhin_models[cache_key] = loaded_model
    twhin_model = _twhin_models[cache_key]
    return twhin_model


def load_model(model_name, available_device="cpu", model_name_or_path=None):
    try:
        if model_name == 'paraphrase-MiniLM-L6-v2':
            return SentenceTransformer(model_name_or_path or model_name,
                                       device=available_device,
                                       cache_folder="./models")
        elif model_name == 'Twitter/twhin-bert-base':
            model_name_or_path = model_name_or_path or model_name
            twhin_tokenizer = get_twhin_tokenizer(model_name_or_path)
            twhin_model = get_twhin_model(available_device,
                                          model_name_or_path)
        ...
        raise Exception(
            f"Failed to load the model: {model_name_or_path or model_name}"
        ) from e


def get_recsys_model(available_device="cpu", recsys_type: str = None,
                     model_name_or_path: str | None = None):
    if recsys_type == RecsysType.TWITTER.value:
        model = load_model('paraphrase-MiniLM-L6-v2', available_device,
                           model_name_or_path)
        return model
        ...
        twhin_tokenizer, twhin_model = load_model(
            ...
            available_device,
            model_name_or_path or "Twitter/twhin-bert-base")
...
def rec_sys_personalized_twh(
    ...
    twhin_tokenizer, twhin_model = get_recsys_model(available_device,
    ...
    if len(post_table) <= max_rec_post_len:
        ...
        post_values = list(t_items.values())
        post_keys = list(t_items.keys())
        filtered_posts_tuple = coarse_filtering(post_values,
                                                coarse_filter_size)
        ...
        for rec_ids in matrix_list:
            rec_ids = [post_keys[i] for i in rec_ids]
