...
MENTION_SCHEMA_SQL = "mention.sql"
INTERACTION_SCHEMA_SQL = "interaction.sql"
...
def create_db(db_path: str | None = None):
    ...
    try:
        ...
        # Read and execute the mention table SQL script:
        mention_sql_path = osp.join(schema_dir, MENTION_SCHEMA_SQL)
        with open(mention_sql_path, "r") as sql_file:
            mention_sql_script = sql_file.read()
        cursor.executescript(mention_sql_script)

        # Read and execute the interaction table SQL script:
        interaction_sql_path = osp.join(schema_dir, INTERACTION_SCHEMA_SQL)
        with open(interaction_sql_path, "r") as sql_file:
            interaction_sql_script = sql_file.read()
        cursor.executescript(interaction_sql_script)
